...
import com.luomor.coco.core.util.bcrypt.BCryptPasswordEncoder;
...
import static com.luomor.coco.admin.util.AdminResponseCode.*;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<CocoAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(CocoAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody CocoAdmin admin) {
        ...
        List<CocoAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        CocoAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody CocoAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody CocoAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        CocoAdmin currentAdmin = (CocoAdmin) currentUser.getPrincipal();
