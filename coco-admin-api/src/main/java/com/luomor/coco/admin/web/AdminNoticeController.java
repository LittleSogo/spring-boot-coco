...
import com.luomor.coco.db.service.CocoNoticeAdminService;
import com.luomor.coco.db.service.CocoNoticeService;
...
import static com.luomor.coco.admin.util.AdminResponseCode.NOTICE_UPDATE_NOT_ALLOWED;
...
public class AdminNoticeController {
    ...
    @Autowired
    private CocoNoticeService noticeService;
    @Autowired
    private CocoAdminService adminService;
    @Autowired
    private CocoNoticeAdminService noticeAdminService;
    ...
    public Object list(String title, String content,
        ...
        List<CocoNotice> noticeList = noticeService.querySelective(title, content, page, limit, sort, order);
        return ResponseUtil.okList(noticeList);
    }

    private Object validate(CocoNotice notice) {
    ...
    private Integer getAdminId(){
        ...
        CocoAdmin admin = (CocoAdmin) currentUser.getPrincipal();
        return admin.getId();
    }
    ...
    @PostMapping("/create")
    public Object create(@RequestBody CocoNotice notice) {
    ...
    public Object read(@NotNull Integer id) {
        CocoNotice notice = noticeService.findById(id);
        List<CocoNoticeAdmin> noticeAdminList = noticeAdminService.queryByNoticeId(id);
    ...
    @PostMapping("/update")
    public Object update(@RequestBody CocoNotice notice) {
        Object error = validate(notice);
        if (error != null) {
            return error;
        }
        CocoNotice originalNotice = noticeService.findById(notice.getId());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody CocoNotice notice) {
        // 1. 删除通知管理员记录
