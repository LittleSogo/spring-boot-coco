...
public class GrouponRuleExpiredTask extends Task {
    ...
    public void run() {
        ...
        CocoGrouponService grouponService = BeanUtil.getBean(CocoGrouponService.class);
        CocoGrouponRulesService grouponRulesService = BeanUtil.getBean(CocoGrouponRulesService.class);

        CocoGrouponRules grouponRules = grouponRulesService.findById(grouponRuleId);
        ...
        List<CocoGroupon> grouponList = grouponService.queryByRuleId(grouponRuleId);
        // 用户团购处理
        for(CocoGroupon groupon : grouponList){
            Short status = groupon.getStatus();
            CocoOrder order = orderService.findById(groupon.getOrderId());
