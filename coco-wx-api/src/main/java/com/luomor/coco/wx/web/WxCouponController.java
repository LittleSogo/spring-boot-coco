...
import com.luomor.coco.wx.vo.CouponVo;
import com.luomor.coco.wx.util.WxResponseCode;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<CocoCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<CocoCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<CocoCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(CocoCouponUser couponUser : couponList){
            Integer couponId = couponUser.getCouponId();
            CocoCoupon coupon = couponService.findById(couponId);
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            CocoCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice);
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        CocoCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
