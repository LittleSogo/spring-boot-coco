...
import com.luomor.coco.core.express.ExpressService;
import com.luomor.coco.core.express.dao.ExpressInfo;
import com.luomor.coco.core.notify.NotifyService;
import com.luomor.coco.core.notify.NotifyType;
...
import com.luomor.coco.core.util.DateTimeUtil;
...
import com.luomor.coco.db.util.OrderHandleOption;
import com.luomor.coco.db.util.OrderUtil;
import com.luomor.coco.core.util.IpUtil;
import com.luomor.coco.wx.task.OrderUnpaidTask;
...
import static com.luomor.coco.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private CocoOrderGoodsService orderGoodsService;
    @Autowired
    ...
    @Autowired
    private CocoCommentService commentService;
    @Autowired
    ...
    @Autowired
    private CocoAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<CocoOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (CocoOrder o : orderList) {
            ...
            CocoGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<CocoOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        CocoOrder order = orderService.findById(userId, orderId);
        if (null == order) {
        ...
        List<CocoOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            CocoGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                CocoGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        CocoAddress checkedAddress = addressService.query(userId, addressId);
        ...
        CocoGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<CocoCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            CocoCart cart = cartService.findById(cartId);
        ...
        for (CocoCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            CocoCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (CocoCart cartGoods : checkedGoodsList) {
            // 订单商品
            CocoOrderGoods orderGoods = new CocoOrderGoods();
        ...
        for (CocoCart checkGoods : checkedGoodsList) {
            ...
            CocoGoodsProduct product = productService.findById(productId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            CocoCouponUser couponUser = couponUserService.findById(userCouponId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            CocoGroupon groupon = new CocoGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                CocoGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        CocoOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        CocoGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            CocoGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<CocoGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (CocoGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                CocoGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<CocoOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        CocoOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        CocoOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        CocoComment comment = new CocoComment();
