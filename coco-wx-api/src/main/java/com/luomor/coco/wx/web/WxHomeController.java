...
public class WxHomeController {
    ...
    @Autowired
    private CocoAdService adService;
    ...
    private List<Map> getCategoryList() {
        ...
        List<CocoCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (CocoCategory catL1 : catL1List) {
            List<CocoCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (CocoCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<CocoGoods> categoryGoods;
            if (l2List.size() == 0) {
